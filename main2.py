...
def get_weather_data(city):
    ...
    if 'main' in response:
        ...
        return weather_details + (
            "The weather in {} is currently {} with a temperature of {} degrees and wind speeds reaching {} kilometres per hour".format(
                weather_data['city'], weather_data['feels_like'], weather_data['temperature'], weather_data['wind']))
    else:
        return None

if __name__ == '__main__':
    speaker = win32com.client.Dispatch("SAPI.SpVoice")
    speaker.Speak("Alexa Playing...,How can I help you")
    while True:
        # print("Enter text")
        # s=input()
        # say(s)
        ...
        sites = [["youtube", "https://www.youtube.com"], ["wikipedia", "https://www.wikipedia.com"],
                 ...
                 ["spotify song", "https://open.spotify.com/track/06uNwpGEx0XElATDWFwsxZ"]]
        for site in sites:
            if f"Open {site[0]}".lower() in text.lower():
                say(f"Opening {site[0]} Kashish...")
                webbrowser.open(site[1])

        apps = [["chrome","C:/Program Files/Google/Chrome/Application/chrome.exe"],["Packet Tracer","C/Program Files/Cisco Packet Tracer 8.2.1/bin/PacketTracer.exe"],["whatsapp","C:/Program Files/WindowsApps/5319275A.WhatsAppDesktop_2.2409.8.0_x64__cv1g1gvanyjgm/WhatsApp.exe"]]
        for app in apps :
            if f"open {app[0]}".lower() in text.lower():
                say(f"Opening {app[0]} Kashish...")
                webbrowser.open(app[1])
        if "the time" in text:
            strfTime = datetime.datetime.now().strftime("%H:%M:%S")
            say(f"Kashish the time is {strfTime}")
        ...
        if "send email" in text:
            ...
            send = server.sendmail('priyanshisojitra1712@gmail.com', 'kashishsavaliya10@gmail.com',
                                   'Hii Kashish , Greetings from Priyanshi...')
            say(send)
